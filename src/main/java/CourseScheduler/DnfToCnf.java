...
class simplification extends logic_base{
	...
	public Boolean run(){
		...
		//I know there's a warning here, but please don't delete; since I didn't write this code,
		//		I don't wanna risk it not working
...
public class DnfToCnf {
	 ...
	 * into originalString. It also converts all the symbols to word versions of the symbols.
	...
	public String stringReplacerA(String string_Original)
	{
		...
		{
			//End of String; Break
			if (string_Original.length() == currentChar)
				break;
			
			//Get Current Character
			char tempC = string_Original.charAt(currentChar);
			char nextC = string_Original.charAt(currentChar);
			if ( string_Original.length() > (currentChar + 1) )
				nextC = string_Original.charAt(currentChar + 1);
			char tempCZ = tempC;
			char nextCZ = '0';
			
			//If Parentheses Or Symbols, Mark tempC As '!' (For Easy of if)
			...
			//If Symbol, Add to newString Only and Replace with ands/ors
			if (tempCZ == '!')
			{
				if ( (tempC == '[') || (tempC == '{') )
					tempC = '(';
				...
				if (tempC == '|')
					addon = " or ";
				newString = (newString + addon);
			}
			
			//If Not a Symbol, Add to newString and originalString
			if (tempCZ != '!')
			{
				currentCourse = (currentCourse + tempC);
				if (nextCZ == '!')
				{
					originalString.add(currentCourse);
					currentCourse = "";
	...
	/**
	 * This transforms every occurrence of a Course back into the original Course name, using originalString. It also
	 * converts occurrences of ands and ors back into symbols again.
	 * @param originalString The original String
	...
	public String stringReplacerB(String string_Original)
	{
		...
		while (true)
		{
			...
			//If Not A->Z, Add to newString Only
			int isLetter = (int) tempC;
			if ( (isLetter < 65) || (isLetter > 90) )
				newString = (newString + tempC);
			
			//Is a Letter; Replace with originalString Values
			else
			{
				int index = (isLetter - 65);
				String currentCourse = originalString.get(index);
				newString = (newString + currentCourse);
			}
			currentChar++;
		}
	...
	/**
	 * Our modifications to the original's main() method; now is a regular method that's passed from RunAlgorithm.java.
	 * @param input The input to be converted to CNF type
	 * @return conversionB The conversion of CNF of the input
	 */
    public String rearrangeString(String input) {
    	//My Stuff
    	String inputFile = "DnfToCnf_Input.txt";
    	String outputFile = "DnfToCnf_Output.txt";
    	DnfToCnf test = new DnfToCnf();
    	String conversionA = test.stringReplacerA(input);
    	//Write to File
        try (FileWriter writer = new FileWriter(inputFile);
                BufferedWriter bw = new BufferedWriter(writer)) {

              bw.write(conversionA);
    	...
    	String conversionB = test.stringReplacerB(result);
		return conversionB;
    }
}